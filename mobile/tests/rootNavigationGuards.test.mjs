...
// back to the first screen whose guard allows it. `elegir-modo` is declared first, so its
...
test('the mode chooser is only mounted while a mode choice is pending', () => {
  const guard = layout.match(
    /<Stack\.Protected guard=\{([^}]+)\}>\s*<Stack\.Screen name="elegir-modo"/,
  );
  assert.ok(guard, 'elegir-modo must be wrapped in its own Stack.Protected');
