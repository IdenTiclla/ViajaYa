...
def markdown(texto):
    ...
    for linea in texto.splitlines():
        ...
        if linea.startswith('|'):
            ...
            salida.append('<li>' + en_linea(linea[2:]) + '</li>')
        elif linea.startswith('#'):
...
def slide(titulo, cuerpo, clase='', etiqueta='PLAN DE PRODUCCIÓN'):
    ...
    diapositivas.append(f'<section class="slide {clase}" id="slide-{numero}" aria-labelledby="titulo-{numero}" data-title="{html.escape(titulo, quote=True)}"><div class="slide-inner"><header><p class="eyebrow">{etiqueta}</p><h2 id="titulo-{numero}" tabindex="-1">{titulo}</h2></header><div class="slide-body">{cuerpo}</div><footer><span>ViajaYa · Bolivia / expansión internacional</span><span>{numero:02d}</span></footer></div></section>')
...
def frente(num, titulo, items, cierre, nota=''):
    slide(titulo, '<div class="frente"><div class="numero-frente">' + f'{num:02d}' + '<span>FRENTE DE TRABAJO</span></div><div>' + puntos(items) + '</div></div><p class="criterio"><strong>Resultado esperado</strong> ' + en_linea(cierre) + '</p>' + (f'<p class="fuente">{nota}</p>' if nota else ''), etiqueta=f'PENDIENTES · FRENTE {num:02d} DE 10')


slide('ViajaYa<br>Salida a producción', '<p class="lead">El trabajo pendiente para abrir en Bolivia<br>y preparar la expansión a otros países.</p><p class="cover-meta">Android · Taxi, moto y encomiendas</p><p class="cover-date">9 de septiembre de 2026 · Revisión 4 · OTP sin SMS en desarrollo y pruebas</p>', 'cover', 'HOJA DE RUTA')
...
nombres = ['Operación y proveedores', 'Teléfono, OTP y seguridad', 'Panel y feature flags', 'Expansión territorial', 'Conductores y soporte', 'Navegación, GPS y mapas', 'Pagos y liquidaciones', 'Encomiendas', 'Infraestructura y alertas', 'Privacidad y Google Play']
slide('Diez frentes pendientes', '<div class="agenda">' + ''.join(f'<div><span>{i:02d}</span><p>{t}</p></div>' for i,t in enumerate(nombres,1)) + '</div><p class="fuente">Cada frente se entrega en PR con pruebas y criterios de cierre. No se asignan fechas sin acordar equipo y disponibilidad.</p>')

frente(1, 'Operación y proveedores', [
    'Definir zonas, servicios, horarios y responsables de soporte.',
    'Revisar transporte, seguros, contratos, impuestos y facturación con asesoría local.',
    'Certificar una pasarela QR con consulta, notificaciones, devoluciones y liquidaciones.',
    'Configurar comisión, calendario de liquidación, cancelaciones y límites de deuda.',
    'Preparar cuentas empresariales, dominio, correo y Google Play.'
], 'Zonas con responsables y proveedor de pagos con contrato y entorno de pruebas confirmados.')

frente(2, 'Acceso por teléfono y OTP', [
    'Unificar login/registro en Continuar con teléfono, sin contraseña para el flujo nuevo.',
    'Google y Facebook opcionales; verificar el número por OTP antes de operar.',
    'Una cuenta estable, teléfono verificado único y vinculación social explícita.',
    'Migrar usuarios conservando historial y rol; ofrecer cambio de número y recuperación.',
    'OTP con caducidad, uso único y límites; sesiones revocables y panel con acceso reforzado.'
], 'Entrada sencilla sin cuentas duplicadas ni pantallas de recuperación bloqueadas.')

slide('Un solo flujo para entrar o registrarse', '<div class="triple"><div><h3>Con teléfono</h3>' + puntos(['Elegir país y número; +591 inicial.', 'Verificar OTP; SMS real solo en producción.', 'Entrar o completar nombre y términos.']) + '</div><div><h3>Con Google o Facebook</h3>' + puntos(['Validar la cuenta social.', 'Solicitar teléfono y OTP si no está verificado.', 'Confirmar la vinculación y completar datos.']) + '</div><div><h3>Al volver</h3>' + puntos(['Conservar la sesión válida.', 'No enviar SMS en cada apertura o viaje.', 'Reverificar al cambiar número, recuperar acceso o por seguridad.']) + '</div></div><p class="criterio"><strong>Migración segura</strong> Mantener ID, historial y rol; no fusionar por correo o teléfono antiguo sin verificar. Retirar el acceso antiguo después de la transición.</p>', etiqueta='PENDIENTES · CONTINUACIÓN DEL FRENTE 02')

slide('OTP sin costo en entornos bajos', '<div class="triple"><div><h3>Desarrollo y pruebas</h3>' + puntos(['El backend genera un desafío simulado.', 'La app autocompleta el código y lo identifica como prueba.', 'Sin envíos, verificaciones externas ni credenciales SMS.']) + '</div><div><h3>Validación completa</h3>' + puntos(['Continuar verifica el desafío en el backend.', 'Conservar caducidad, límites y uso único.', 'Desactivar autofill para ensayar errores y reenvíos.']) + '</div><div><h3>Producción aislada</h3>' + puntos(['OTP real con el proveedor elegido.', 'No devolver códigos ni aceptar el simulador.', 'El build excluye la ayuda de autocompletado de prueba.']) + '</div></div><p class="criterio"><strong>Costo de proveedor: 0 USD</strong> Aplica a OTP en desarrollo/pruebas; su hosting sigue presupuestado. CI debe comprobar cero llamadas externas y separación de producción.</p>', etiqueta='PENDIENTES · CONTINUACIÓN DEL FRENTE 02')

frente(3, 'Panel y feature flags', [
    'Revisar conductores, vehículos, documentos, viajes e incidentes.',
    'Gestionar cobros, comisiones pendientes, liquidaciones y excepciones auditadas.',
    'Permisos separados para administración, soporte y finanzas; autenticación reforzada.',
    'Flags aplicadas por backend para país, ciudad, servicio, pago y grupo de usuarios.',
    'Mostrar disponibilidad e indicadores de operación y gasto.'
], 'Apagar una función impide operaciones nuevas y conserva la finalización de viajes y pagos activos.')

frente(4, 'Bolivia y expansión internacional', [
...
frente(5, 'Conductores y seguridad del servicio', [
...
frente(6, 'Seguimiento GPS y notificaciones', [
...
frente(9, 'Infraestructura y despliegue', [
...
slide('Capacidad y condiciones de apertura', '<div class="stats"><div><strong>≤ 500 ms</strong><span>API propia · percentil 95</span></div><div><strong>≤ 2 s</strong><span>evento realtime visible · p95</span></div><div><strong>2×</strong><span>ráfagas sobre la carga objetivo</span></div></div><div class="two-col"><div>' + puntos(['Carga sostenida: 500 conductores y, como hipótesis, 500 pasajeros.', 'Sin asignaciones dobles, cobros duplicados ni cancelaciones falsas.']) + '</div><div>' + puntos(['Restauración y rollback ensayados; alerta recibida.', 'Conductores aprobados, soporte y conciliación por zona.', 'Binario aceptado por Google Play.']) + '</div></div><p class="fuente">Objetivos de aceptación propuestos; deben medirse en el entorno desplegado.</p>')

slide('Secuencia de entrega', '<div class="timeline"><div><span>01</span><h3>Fundamentos</h3><p>Proveedores y operación en paralelo con seguridad, panel y configuración territorial.</p></div><div><span>02</span><h3>Servicio completo</h3><p>Conductores, GPS, mapas, pagos y encomiendas. Integración y certificación en el entorno de pruebas.</p></div><div><span>03</span><h3>Certificar y abrir</h3><p>Privacidad, tienda, carga y recuperación. Pruebas cerradas y apertura pública por zonas.</p></div></div><p class="criterio"><strong>Ampliación</strong> Más cobertura y capacidad según estabilidad, demanda y costo observado. Cada país nuevo se certifica por separado.</p>')
