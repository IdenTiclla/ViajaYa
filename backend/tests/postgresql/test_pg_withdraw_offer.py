...
from app.api.deps import get_accept_offer, get_withdraw_offer
...
async def _insert_scenario(sessions):
    async with sessions() as session:
        ...
        rider = await users.add(
            User(
                full_name="Pasajero carrera retiro",
                email=f"rider-withdraw-race-{uuid.uuid4()}@test.com",
            )
        )
        driver = await users.add(
            User(
                full_name="Conductor carrera retiro",
                email=f"driver-withdraw-race-{uuid.uuid4()}@test.com",
...
async def test_withdraw_and_accept_have_one_winner_and_one_durable_outcome(
    pg_test_db,
) -> None:
    ...
    async with sessions() as accept_session, sessions() as withdraw_session:
        accept = get_accept_offer(
            SqlAlchemyRideRequestRepository(accept_session),
            accept_session,
            _settings(),
        )
        withdraw = get_withdraw_offer(withdraw_session, _settings())
        ...
        async def run_withdraw():
            await barrier.wait()
            return await withdraw.execute(driver, offer.id)

        accepted, withdrawn = await asyncio.wait_for(
            asyncio.gather(
                run_accept(),
                run_withdraw(),
    ...
    withdraw_won = isinstance(withdrawn, Offer)
    assert accept_won is not withdraw_won
    ...
    if accept_won:
        assert isinstance(withdrawn, InvalidRideTransitionError)
        assert offer_status is OfferStatus.ACCEPTED
        assert ride_status is RideStatus.ACCEPTED
        assert "offer_withdrawn" not in event_types
        assert event_types == [
            "ride_status",
            "ride_closed",
            "offer_accepted",
